...
//
// `apps/app-catalog.js` is what the registry falls back to when `apps/registry.json` cannot
// be fetched. It used to be written by hand, and it rotted exactly the way a hand-written
// list of file paths rots: `monaco` and `tinymce` gained vendored editors of 98 and 137
// files, the catalog went on naming two each, and installing either from the fallback
// produced an app that opened and stayed blank. Four apps added later were not in it at all.
//
// Now it is written from the same manifests as `registry.json`, in the manifest's own shape,
// so it cannot say something different from them. `base` — the bootstrap file list — comes
// from `settings/preinstall.json`, for the same reason: that file is the modern answer to
// "what does a fresh system need", and two hand-kept copies of it is one too many.
//
// An entry only appears here if its app directory exists and its manifest names an entry
// point. A library bundle that is not an app (`apps/ffmpeg.0.12.10`, 31 MB, loaded by
// Explorer if it happens to be in the filesystem) has no manifest and no install route, and
// listing it here only ever suggested otherwise.
function buildCatalogFromDirs (appDirs) {
	var catalog = {};
	appDirs.forEach(function (meta, appDir) {
		var manifestPath = path.join(appDir, 'pixos.app.json');
		if (!fs.existsSync(manifestPath)) {
		...
		var entry = Object.assign({}, manifest);
		// The id is the key, and `manifestPath` is how a *registry* entry is found — there
		// is no registry when this file is being read.
		delete entry.id;
		delete entry.manifestPath;
		catalog[manifest.id || meta.id] = entry;
	});
	return catalog;
}
...
function baseEntry () {
	...
	var preinstall = JSON.parse(fs.readFileSync(preinstallPath, 'utf8'));
	var files = (preinstall.files || []).map(function (item) {
		return item.path;
	}).filter(Boolean);
	// No entryPath, deliberately: `base` is a list of files a fresh system needs, not
	// something anybody installs.
	return {name: 'base', files: files};
}

function writeCatalog (appDirs) {
	var catalog = buildCatalogFromDirs(appDirs);
	var base = baseEntry();
	var ordered = {};
	if (base) {
		ordered.base = base;
	...
	var body = JSON.stringify(ordered, null, '\t').split('\n').map(function (line, index) {
		return index === 0 ? line : '\t' + line;
	}).join('\n');

	fs.writeFileSync(CATALOG_JS, header + body + ';\n}());\n');
	return ordered;
}
...
function main () {
	...
	// Written from the manifests on disk rather than from the ones this run happened to
	// touch, so `--only=<id>` still leaves the fallback agreeing with all of them.
