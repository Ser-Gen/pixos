...
// cfg: {host, wm, launch, commands, listFiles, openPath}
...
export function open (initialQuery) {
	...
	input.placeholder = 'Search apps and windows, or type / for files';
	...
	element.append(input, list, hint);
	host.append(element);
	input.focus();
	refresh();
}
	...
	return entries
		...
		.map(function (item) {
			...
			return {
				...
				subtitle: full,
				badge: item.entry.isDirectory ? 'folder' : 'file',
				run: function () {
